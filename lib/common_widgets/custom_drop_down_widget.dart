...
class CustomDropdownWidgetState extends State<CustomDropdownWidget> {
  ...
  fetchInitialization() async{

    optionList.clear();
    String defaultOption = "Choose type of verification";
    optionList.add(defaultOption);
    optionList.clear();
    // List<String> relationshipListFetched = await NetworkRequests().retrieveRelationshipsRequest();
    List<String> relationshipListFetched = ["National ID", "Passport"];
    optionList = [defaultOption, ...relationshipListFetched];
    updateSelection();
